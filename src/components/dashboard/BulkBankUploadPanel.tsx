...
export function BulkBankUploadPanel({ isOpen, onClose, onUpload }: BulkBankUploadPanelProps) {
  ...
  const downloadTemplate = () => {
    const headers = ['Cuenta', 'Monto', 'Fecha'];
  ...
  const handleProcess = () => {
    try {
      ...
      const startIndex = (rows[0].toLowerCase().includes('cuenta') || rows[0].toLowerCase().includes('monto')) ? 1 : 0;
      ...
      const parsedData = rows.slice(startIndex).map(row => {
        ...
        return {
          account: columns[0] || '-',
          amount: columns[1] || '0',
          date: columns[2] || new Date().toLocaleDateString('es-MX')
        };
      });
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
              <p className="text-[10px] text-blue-800 leading-relaxed font-normal">
                ...
                <strong>Cuenta | Monto | Fecha</strong>
              </p>
            </div>
